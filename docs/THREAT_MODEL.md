...
| **Denial of Service** | Lyzr API or AIMS telemetry outage blocks local screening completion | Bounded exponential backoff on Lyzr client (`errors.ts`). Transactional AIMS outbox isolates external telemetry failure | `test_aims_failure_does_not_fail_screening` & `test_aims_503_outage_preserves_screening_completed` | **LOW** |
