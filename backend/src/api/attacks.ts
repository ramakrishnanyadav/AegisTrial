...
const handleAttackExecution = async (req: Request, res: Response): Promise<void> => {
  ...
  const rawId = req.params['id']?.toUpperCase() || '';

  let attackId: AttackId | null = null;
  if (rawId === '1' || rawId === 'ATTACK-01' || rawId === 'ATTACK-1') attackId = AttackId.PROMPT_INJECTION;
  else if (rawId === '2' || rawId === 'ATTACK-02' || rawId === 'ATTACK-2') attackId = AttackId.MISSING_EVIDENCE;
  else if (rawId === '3' || rawId === 'ATTACK-03' || rawId === 'ATTACK-3') attackId = AttackId.UNIT_CORRUPTION;
  else if (rawId === '4' || rawId === 'ATTACK-04' || rawId === 'ATTACK-4') attackId = AttackId.CONFLICTING_LABS;
  else if (rawId === '5' || rawId === 'ATTACK-05' || rawId === 'ATTACK-5') attackId = AttackId.AI_FREE_REPLAY;
  else if (rawId === '6' || rawId === 'ATTACK-06' || rawId === 'ATTACK-6') attackId = AttackId.AIMS_OUTAGE;
...
[END OF EHR]`;
      ...
      const safePatientFields: PatientField[] = [
        { field: 'anc', value: 1800, unit: 'cells/μL', timestamp: new Date().toISOString(), confidence: 0.95, sourceExcerpt: 'ANC: 1800 cells/uL' },
      ];

      const resolved = resolveEvidence(NSCLC_CRITERIA[0]!, safePatientFields);
    ...
    case AttackId.AI_FREE_REPLAY: {
      let recent = listRecentScreeningRuns(1);
